...
def CreateOverDetTvarSet(thisStateList,thisTvarList):
    outlist = []
    for itvar in thisTvarList:
        outlist += ['state'+istate+itvar for istate in thisStateList]
    return outlist
...
def ReduceTooMassSet(thisSetList):
    ...
    for iset in notsinkList:
        if 'sm' in iset and 'state' not in iset and 'tsrc' not in iset:
