...
def AppendSummaryDat(xdata,ydata,yerr):
    ...
    with open(DatFile,'a') as f:
        for ix,iy,iyerr in zip(xdata,ydata,yerr):
            f.write('{0:>3}  {1:10}'.format(ix,MakeValAndErr(iy,iyerr)))
...
def GraphCondit(iDS,igamma,iq,thisMeth,iSet):
    ...
    if 'Fits' in thisMeth and 'PoF' in iSet: graphthis = False
...
def CreateMethodSetList(thisMethodList,setlist):
    ...
    for iMeth in thisMethodList:
        if 'Fits' in iMeth :
            ...
            mslout[iMeth] = []
            for iset in setlist:
                if (not 'state' in iset) and iMeth in iset:
                    mslout[iMeth].append(iset.replace(iMeth,''))                    
        else:
            mslout[iMeth] = []
            for iset in setlist:
            ...
            if iSet in datakey.replace(methcomp,'') and methcomp in datakey: 
                if iDS == False:
                    ...
                    if 'Avg' in idata[igamma][iq].keys() and 'Std' in idata[igamma][iq].keys():
                        dataval.append(idata[igamma][iq]['Avg'])
                        dataerr.append(idata[igamma][iq]['Std'])
                    else:
                        dataval.append(idata[igamma][iq]['Boot'].Avg)
                    ...
                    if 'Avg' in idata[iDS][igamma][iq].keys() and 'Std' in idata[iDS][igamma][iq].keys():
                        dataval.append(idata[iDS][igamma][iq]['Avg'])
                        dataerr.append(idata[iDS][igamma][iq]['Std'])
                    else:
                        dataval.append(idata[iDS][igamma][iq]['Boot'].Avg)
