...
def PlotFitMethPN(currdata,Ncurrdata,thiscurr,thisSetList):
    ...
    if 'Top' in thiscurr:
        for iset in DefSetList:
            for icut in FitCutPlot:
                PlotFFWrapPN(currdata,Ncurrdata,thiscurr,FlagList(thisSetList,'Fit',iset,icut),'Fit'+iset+icut+'FlowComp')
        for icut in FitCutPlot:
            for iflow in FlowPlot:
        ...
        for icut in FitCutPlot:
            PlotFFWrapPN(currdata,Ncurrdata,thiscurr,FlagList(thisSetList,'Fit',icut),'Fit'+icut+'SetComp')
...
def PickFFFewSets(currdata,thiscurr,thisSetList):
    PickedSetList = []
    if kappa == 12090:
        PickedSetList += FlagList(thisSetList,'OSFCM','tsink29sm32',FitCutPicked['tsink29sm32'])
        PickedSetList += FlagList(thisSetList,'OSFCM','tsink29sm64',FitCutPicked['tsink29sm64'])
        PickedSetList += FlagList(thisSetList,'OSFCM','tsink29sm128',FitCutPicked['tsink29sm128'])
        PickedSetList += FlagList(thisSetList,'OSFCM','tsink29state1CMto20dt2',FitCutPicked['tsink29state1CM'])
...
for iFFcomb in feedin['FFcomb']:
    if iFFcomb == '':
        for icurr in ElongateName(feedin['comb'],feedin['current']):
            ...
            if 'ProtonVectorTop' in icurr or 'ProtonVectorWein' in icurr:
