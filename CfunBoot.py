        ...
        try:
            if CHROMA:
                ...
                if NoXAvg:
                    ...
                    tempdataTop.append([])
                    tempdata.append(data.data)
                    print np.array(chargedata).shape, chargeindex
                    for iflowdata in chargedata[chargeindex]:
                        tempdataTop[-1].append(np.array(data.datag5)*iflowdata)
...
def PlotAutoCorr(NNdata,NNQdata,TorFlow,NNboot,NNQboot,flowlist=[]):
    ...
    pl.errorbar(np.arange(len(AlphaBoot))+thisshift,Pullflag(AlphaBoot,'Avg'),Pullflag(AlphaBoot,'Std'),fmt='.',label='Bootstrap')
    pl.ylabel(r'$ \alpha$')
    pl.legend()
