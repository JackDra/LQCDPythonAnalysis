...
def GraphWchit(Wlist,flowlist):
    ...
    print 'len(taulist)',len(flowlistlist)
    ...
    pl.clf()
    pl.errorbar(flowlist,meanlist,alphaerr,fmt='.',label='Autocorr')
    W2boot,dump = bt.CreateBoot(np.array(Wlist)**2,nboot,0)
    pl.errorbar(flowlist+thisshift,Pullflag(W2boot,'Avg'),Pullflag(W2boot,'Std'),fmt='.',label='Bootstrap')
    pl.ylabel(r'$<W^{2}>$')
    pl.legend()
    pl.title(r'$<W^{2}>$')
    pl.xlabel(r'$t_{flow}$')
