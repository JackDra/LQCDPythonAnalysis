...
ylimFFDict = {'ProtonVectorTopFF3':[-2,2],'NeutronVectorTopFF3':[-2,2]}
...
def SetFFAxies(thisCurr):
    ...
    pl.xlabel(FFxlab)
    if currnumb == '3':
        print FFylab.replace('F','\\frac{F_{'+currnumb+'}')+'}{2m_{N}}'
        pl.ylabel(FFylab.replace('F','\\frac{F_{'+currnumb+'}')+'}{2m_{N}}')
    else:
        if 'GeGm' in thisCurr:
            if currnumb == '1':
                currnumb = 'E'
...
def PlotDPFit(thisset,thisFF,thisCurr,thiscol,qrange,thisshift,flipsign,datf,thiskappa=kappa):
    ...
    if 'FF1' in thisFF:
        ...
        LegVal = '$\\frac{F_{3}(0)='+MakeValAndErr(Avg[1],Err[1])+'}{2m_{N}}$'        
