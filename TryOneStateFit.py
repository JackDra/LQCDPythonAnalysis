...
def DoOSF(thisSetList,thisGammaList,OSF2ptarray,twoptGammaMomList,thisMomList):
    ...
    if Debug: print 'Debugging: OSF ' + thisGammaList[0]+' ' + thisMom + ' took ' , str(datetime.timedelta(seconds=time.time()-totstart)) , ' h:m:s'
...
if os.path.isfile(picklefile2pt):
    ...
    [dump,data2pt,twoptGammaMomList,dump3,dump4,infolist2pt] = ReadCfunsnp(['twopt'],twoptSL,thisMomList=feedin['mom'])
    ...
    PrintOSFMassToFile(OneFit2pt,OneFit2ptChi,twoptSL,thisFitOSFR,outfile,twoptGammaMomList['twopt'],infolist2pt)
...
for igamma in ReadGammaList:
    ...
    if 'doub' not in igamma and 'sing' not in igamma:
        if DefWipe:
            ...
            QueMomList = Check3ptAllSets(['doub'+igamma,'sing'+igamma],ReadSetList,thisMomList=feedin['mom'],CheckType='OSF'+outfile)
            QueMomList = QueMomList[igamma]
        for imom in QueMomList:
            # print 'adding to que: ' , igamma , imom
            inputparams.append((ReadSetList,['doub'+igamma,'sing'+igamma],OSF2ptarray,twoptGammaMomList,[imom]))
